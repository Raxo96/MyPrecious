#!/usr/bin/env python3
"""
Test script for RateLimiter component.
Tests basic functionality: minimum delay enforcement, hourly limit, exponential backoff.
"""
import os
import sys
import time
...
def test_rate_limiter():
    ...
    print("=" * 60)
    print("Testing RateLimiter Component")
    print("=" * 60)
    ...
    try:
        limiter = RateLimiter(min_delay_seconds=0.5, hourly_limit=1800)
        ...
        assert second_request_time >= 0.45, f"Expected delay >= 0.45s, got {second_request_time:.3f}s"
        print(f"✓ Minimum delay of 0.5s was enforced")
    ...
    try:
        ...
        try:
            for attempt, expected_delay in enumerate(expected_delays, start=1):
                ...
                print(f"  Attempt {attempt}: expected={expected_delay}s, actual={actual_delay}s")
    ...
    # Test 6: Hourly limit enforcement (simulated)
    print("\n[Test 6] Testing hourly limit enforcement (simulated)...")
    try:
        # Use a very small hourly limit for testing
        limiter = RateLimiter(min_delay_seconds=0.01, hourly_limit=3)
        ...
        count = limiter.get_hourly_count()
        print(f"✓ Made 3 requests (limit=3), counter shows: {count}")
        
        # The 4th request should trigger a wait
        # We'll test that wait_if_needed would pause, but we won't actually wait
        # (to keep test fast)
        print("✓ Hourly limit reached (3/3)")
        print("  Note: Full hourly limit pause test skipped for speed")
        
    except Exception as e:
        print(f"✗ Hourly limit test failed: {e}")
        return
    
    # Test 7: get_hourly_count method
    print("\n[Test 7] Testing get_hourly_count()...")
    try:
        limiter = RateLimiter(min_delay_seconds=0.01, hourly_limit=1800)
